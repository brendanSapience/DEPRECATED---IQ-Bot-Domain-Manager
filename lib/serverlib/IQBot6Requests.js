...
exports.ImportDomainRequest = function(host,authport,aliasport,authdata,domaindata,socket){
	...
	genws.SendGenericPostHttpRequest(socket, host,authport,authpath,authdata,function(AuthRes){
		...
		if(token == ""){
			...
			genws.SendGenericPostHttpRequestWithToken(socket,host,aliasport,domainimportpath,domaindata,token,function(DomainImportRes){
			//console.log("gaga:"+DomainImportRes);
